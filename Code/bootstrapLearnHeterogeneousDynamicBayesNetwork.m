function DBN = bootstrapLearnHeterogeneousDynamicBayesNetwork(filename, pheno, nboots, edgeThreshold, cols2learn, maxParents, localNetStruc, intraEdges, mle, nu, alpha, sigma2, BFTHRESH, MAXDISCVALS, verbose,matrixName)
...
% G = bootstrapLearnHeterogeneousDynamicBayesNetwork('infant_gut_microbiota_dbn_sample_alignment_filtered_sr5d', '', 10, 0.7, 5, false, false, false, 1, 1, 1); %MAP
    ...
    [interactionMatrix, interactionTypes] = RCSVLoad([matrixName, '.matrix'], false, '\t');
...
%                DBN.WriteToTGF(dbn_filename);
    ...
    tEnd = toc(tStart)  
    dbn_filename = ['../networks/DBN_',filename,'_nboots', int2str(nboots),'_nParents',int2str(maxParents),'_matrix',matrixName,'_s',int2str(round(tEnd))];

    fprintf(dbn_filename)
